...
import sys
import shlex # For quoting display
from typing import List, Dict, Optional, Tuple, Any

# Modules from our package
try:
    # Import Monitor class and FileInfo for type hints if needed
    from monitor import Monitor, FileInfo # The state management class
    # Import Syscall class and the parser/streamer generator
    from strace_cmd import Syscall, parse_strace_stream, DEFAULT_SYSCALLS
except ImportError as e:
    # Provide a more informative error if the import fails
    print(f"ERROR: Failed to import 'Monitor' from 'monitor.py' or "
          f"'Syscall'/'parse_strace_stream' from 'strace_cmd.py'. "
          f"Ensure these files exist and are importable. Details: {e}", file=sys.stderr)
    # Exit here if dependencies are missing
...
def _process_syscall_event(
    ...
    if pid is None:
        ...
        # For now, assume TID=PID if not found. This is a potential
        # inaccuracy if attaching mid-way to a non-main thread.
        pid = tid
    ...
    # Needs to happen *before* calling handlers, as handlers might use the map implicitly
    if syscall_name in ["clone", "fork", "vfork"] and success:
        try:
            new_id = result_code
            if new_id is not None and new_id > 0: # Parent context receiving child ID
                ...
                if new_id not in tid_to_pid_map:
                     ...
                     log.info(f"Syscall {syscall_name}: Mapped new TID/PID {new_id} to parent PID {pid}")
    ...
    try:
        # Prepare details dict (common to most handlers)
        if not success and error_name: details["error_name"] = error_name
        # Add byte count for I/O syscalls
        if syscall_name in ["read", "pread64", "readv", "write", "pwrite64", "writev"] and success and result_code is not None:
            # Read can return 0 for EOF, Write usually > 0
            if result_code >= 0: details["bytes"] = result_code

        # --- Map syscall to Monitor method ---
        # Pass pid, but NOT tid to the monitor handlers
        handler_method = None
        handler_args: Tuple = () # Define type for clarity
        ...
        if syscall_name in ["open", "creat"]:
            ...
            else: log.warning(f"Skipping {syscall_name} event, missing path or result: {event!r}")

        elif syscall_name == "openat":
             path = _clean_path_arg(args[1] if len(args) > 1 else None)
             # TODO: Maybe resolve first arg (dirfd) like AT_FDCWD? Complex.
             if path is not None and result_code is not None:
                 fd = result_code if success else -1
                 handler_method = monitor.open
                 handler_args = (pid, path, fd, success, timestamp)
             else: log.warning(f"Skipping openat event, missing path or result: {event!r}")

        elif syscall_name in ["read", "pread64", "readv"]:
            if len(args) > 0: fd = _parse_result(str(args[0]))
            if fd is not None:
                 # Resolve path using monitor's cache before calling handler
                 path = monitor.get_path(pid, fd)
                 handler_method = monitor.read
                 handler_args = (pid, fd, path, success, timestamp) # Pass resolved path (or None)
            else: log.warning(f"Skipping read event, missing fd: {event!r}")
            ...
            if fd is not None:
                 ...
                 handler_args = (pid, fd, path, success, timestamp) # Pass resolved path (or None)
            else: log.warning(f"Skipping write event, missing fd: {event!r}")
            ...
            else: log.warning(f"Skipping close event, missing fd: {event!r}")
            ...
            else: log.warning(f"Skipping {syscall_name} event, missing path: {event!r}")

        elif syscall_name == "newfstatat":
             # FD is arg 0, Path is arg 1
             # fd = _parse_result(str(args[0])) # We don't necessarily need the dirfd
             path = _clean_path_arg(args[1] if len(args) > 1 else None)
             # TODO: Resolve path relative to dirfd if not absolute? Complex.
             if path is not None:
                  handler_method = monitor.stat
                  handler_args = (pid, path, success, timestamp)
             else: log.warning(f"Skipping newfstatat event, missing path: {event!r}")
            ...
            if path is not None:
                 ...
                 handler_args = (pid, path, success, timestamp)
            else: log.warning(f"Skipping unlink event, missing path: {event!r}")
             ...
             if path is not None:
                  handler_method = monitor.delete
                  handler_args = (pid, path, success, timestamp)
             else: log.warning(f"Skipping unlinkat event, missing path: {event!r}")
             ...
             else: log.warning(f"Skipping rename event, missing paths: {event!r}")
             ...
             if old_path and new_path:
                  handler_method = monitor.rename
                  handler_args = (pid, old_path, new_path, success, timestamp)
             else: log.warning(f"Skipping renameat event, missing paths: {event!r}")

        # Call the handler if found
        if handler_method:
            # Pass tid=tid explicitly? No, removed from Monitor handlers.
            # Pass details dictionary
            handler_method(*handler_args, **details)
        elif syscall_name not in PROCESS_SYSCALLS: # Don't log process calls as unhandled file ops
...
def attach(pids_or_tids: List[int], monitor: Monitor, syscalls: List[str] = DEFAULT_SYSCALLS):
    """
    ...
    for initial_tid in pids_or_tids:
        try:
            # Check existence first to avoid warnings on expected missing TIDs if a process group was given
            ...
            # Should be caught by pid_exists, but handle race condition
            log.warning(f"Process/Thread with TID {initial_tid} disappeared during initial mapping.")
        except psutil.AccessDenied:
            log.warning(f"Access denied getting PID for initial TID {initial_tid}. Mapping might be incomplete.")
        except Exception as e:
    ...
    log.info(f"Starting event processing loop for attached PIDs/TIDs (maps to PIDs: {list(initial_pids)})...")
    try:
        # Pass the combined list of syscalls needed by parser and this adapter
        combined_syscalls = sorted(list(set(syscalls) | set(PROCESS_SYSCALLS)))
        for syscall_event in parse_strace_stream(attach_ids=pids_or_tids, syscalls=combined_syscalls):
...
def run(command: List[str], monitor: Monitor, syscalls: List[str] = DEFAULT_SYSCALLS):
    ...
    try:
        # Pass the combined list of syscalls needed by parser and this adapter
        combined_syscalls = sorted(list(set(syscalls) | set(PROCESS_SYSCALLS)))
        for syscall_event in parse_strace_stream(target_command=command, syscalls=combined_syscalls):
             _process_syscall_event(syscall_event, monitor, tid_to_pid_map)
    except KeyboardInterrupt:
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Strace adapter: runs/attaches strace and updates Monitor state.",
    ...
    group.add_argument(
        "-p", "--pids", nargs='+', type=int, metavar='PID', # Updated help text
        help="One or more existing process IDs (PIDs) to attach to. Children/threads will be followed."
    )
    ...
    try:
        ...
        log.info("--- Final Monitored State ---")
        # Use the iterator to get files
        tracked_files = list(monitor) # Convert iterator to list
        tracked_files.sort(key=lambda fi: fi.last_activity_ts, reverse=True) # Sort here

        if not tracked_files:
             log.info("No files were tracked.")
        for i, file_info in enumerate(tracked_files):
             # Filter deleted for display here if desired
             # if file_info.status == 'deleted': continue
             print(f"{i+1}: {repr(file_info)}")
             if i > 20: # Limit output for testing
                  print("...")
                  break
        log.info("-----------------------------")
        return 0
         ...
         # Allow finally blocks in generators/adapter to run
         return 130
    except Exception as e:
