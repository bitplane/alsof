...
class FileInfo:
    ...
    event_history: Deque[Dict] = field(
    ...
    )  # Store last error msg, rename target, etc.
...
class Monitor(Versioned):
    ...
    def close(self, pid: int, fd: int, success: bool, timestamp: float, **details):
        ...
        # Early out if path not found in cache or is std stream
        if not path:
            log.info(
                f"Close event for unknown PID {pid} FD {fd}"
        ...
        # Early out if FileInfo doesn't exist (though it shouldn't if path was in cache)
        if not info:
            log.warning(
        ...
        # --- Handle failure case ---
        if not success:
            if info.is_open and info.status != "deleted":
                info.status = "open"
    ...
    def read(
        ...
        **details,
        ...
        if not path:
            log.info(
                f"Read event for PID {pid} FD {fd} could not resolve path."
            )  # Downgraded
            return
        if path in STD_PATHS:
    ...
    def write(
        ...
        **details,
        ...
        if not path:
            log.info(
                f"Write event for PID {pid} FD {fd} could not resolve path."
            )  # Downgraded
            return
        if path in STD_PATHS:
    ...
    def delete(self, pid: int, path: str, success: bool, timestamp: float, **details):
        ...
        for check_pid in pids_to_check:
            if check_pid in info.open_by_pids:
                ...
                for fd in fds_to_remove:
                    ...
                    if (
                        ...
                        and fd in self.pid_fd_map[check_pid]
                    ):
                        if self.pid_fd_map[check_pid][fd] == path:
                            del self.pid_fd_map[check_pid][fd]
                    if (
                        check_pid in info.open_by_pids
                        and fd in info.open_by_pids[check_pid]
                    ):
            ...
            if check_pid in self.pid_fd_map:
                fds_to_remove_extra = {
                    fd for fd, p in self.pid_fd_map[check_pid].items() if p == path
                }
                for fd in fds_to_remove_extra:
                    del self.pid_fd_map[check_pid][fd]
        ...
        **details,
        ...
        if not new_info:
            ...
            self.delete(
                ...
                True,
                timestamp,
                {"error": "Rename target creation failed"},
            )
            return
        ...
        for update_pid in pids_to_update:
            ...
            if fds_to_update:
                log.info(
                    f"Rename: Updating FD map for PID {update_pid}, FDs {fds_to_update}: '{old_path}' -> '{new_path}'"
                )
    ...
    def _finalize_update(
        ...
        # Update details, but ensure 'error_name' from details overrides previous
        # if the current operation failed.
        current_details = info.details
        current_details.update(details)
        if not success and "error_name" in details:
            current_details["error_name"] = details["error_name"]
        elif success and "error_name" in current_details:
            # Clear previous error on success
            del current_details["error_name"]
