...
class Monitor(Versioned):
    ...
    def close(self, pid: int, fd: int, success: bool, timestamp: float, **details):
        ...
        if not path:
            # Changed from INFO to DEBUG for truly unknown FDs
            log.debug(f"Close event for unknown PID {pid} FD {fd}")
            return
        if path in STD_PATHS:
            log.debug(
    ...
    def _finalize_update(
        ...
        if not success and "error_name" in details:
            ...
            if (
                "error_name" not in details
            ):  # Clear previous error only if current event didn't add one
